...
const getMatchedObj = (urlObj, match) => {
...
const replaceUrl = (
  ...
  const matchedObj = getMatchedObj(urlObj, match);

  let urlPath = urlObj.pathname;
  if (matchedObj) {
    const { path, query } = matchedObj;
    if (path.test) {
...
const replaceBody = (
  ...
  if (matchedObj && matchedObj.body && matchedObj.body.ignore) {
    return '';
  }

  const ignoreKeys = (matchedObj.body && matchedObj.body.ignoreKeys) ? matchedObj.body.ignoreKeys : [];
